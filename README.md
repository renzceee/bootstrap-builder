...
Use this repo from a Linux or WSL environment. Clone it first:

```bash
git clone https://github.com/renzceee/bootstrap-builder.git
cd bootstrap-builder
```

Then run the build scripts in order:
...
./4_Build_Bootstrap.sh
```

What each script does:

- `1_config.sh`: Configures the Android package name used by the build scripts and updates the Termux properties script.
- `2_clean.sh`: Removes old build output, local repository files, bootstrap ZIPs, and temporary bootstrap data so the next build starts cleanly.
- `3_Build_Packages.sh`: Builds the core packages needed for the bootstrap. It lets you choose between the standard core package set and a smaller core-minimal set.
- `3.1_Build_Custom_Packages.sh`: Optionally builds extra packages, such as OpenJDK packages, after the core packages are done. Skip this if you only need the default core bootstrap.
- `4_Build_Bootstrap.sh`: Creates the local package repository from built `.deb` files, checks required bootstrap packages, and generates the final bootstrap ZIP.
