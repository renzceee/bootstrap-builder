...
if validate_package_name "$NEW_PKG"; then
    ...
    # Also update the hardcoded name in step1.sh if it exists
    if [ -f "./step1.sh" ]; then
        echo "[*] Syncing new package name to step1.sh..."
        sed -i "s/^CUSTOM_PKG_NAME=.*/CUSTOM_PKG_NAME=\"$NEW_PKG\"/" ./step1.sh
    fi
